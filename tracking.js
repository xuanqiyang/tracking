class Tracking {
     ...
     *   stayPoint: 6,  一组point中被计算为开始停留的点的序号， 序号相同代表一组连续的停留点
     *   startStay: 8   一组point中第一个超过5分钟的停留点的序号
     *
     **/
    constructor(points) {
        ...
        this.points.reduce((leftIndex, v, rightIndex) => {
            ...
            if (ds <= 50) {
                if (
                    ...
                    // 标记第一个超过5分钟的point
                    if (leftIndex === index) {
                        v.startStay = rightIndex;
                    }
                    // 标记一组在停留时间超过5分钟，且运动距离不超过50m的连续point
                    while (index <= rightIndex) {
                        this.points[index].stayPoint = leftIndex;
                        index++;
                    }
                }
                return leftIndex;
    ...
    getStartStayPoints () {
        return this.points.filter((v) => v.startStay !== undefined);
    }
    getStayPoints () {
        return this.points.filter((v) => v.stayPoint !== undefined);
    }
    getReconnectPoints () {
